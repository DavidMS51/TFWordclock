...
#
# Release version 3 - See github [https://github.com/DavidMS51/TFWordclock.git] for usage licence details
...
# Blinking implimented as a startup option with -b suffix 
#[ default 1sec act LED blink,  -1 as TV mod, -2 LED act blink briefly every 10 sec, -3 act led off ]
# Now uses timewrd4ca (with rotation and blinking support) for text.
# Enabling code for German included - but display  not sorted out yet so do not use
...
# Argparse automatically adds: ('-h', '--help', action='store_true', help='show usage')
    ...
    parser.add_argument('language', default='English', nargs='?', choices=['English', 'French', 'Dutch', 'German'], help='Language: English, French, Dutch or  German (default: %(default)s)')
...
# this is NOT implimented yet in timewrd4ca so don't use yet
# just a copy of English at the moment
elif args.language == "German":
        from timewrd4ca  import DMS_wrdck_Ge as DMS_wrdck
...
if args.rotation == 0:
	...
	print "90 degress clockwise"
elif args.rotation == 2:
	print "upside down"
elif args.rotation == 3:
	print "90 degs counterclockwise"
...
while True:
	...
	while cur_time_min == temp:
		...
		while t < 100:
			...
			# Blink display to indicate minute in 5-minute interval except for French and blink optons 0 and 2
					second = t / 10
					if blink < nblinks and args.language != "French" and args.blink == 1:
						if second & 1:
							# blink off
