...
void st_display(void) {
  lc.clearDisplay(0);
  // display version number for approx 1 second
  lc.setLed(0, 0, 6, true);    // display V

  lc.setLed(0, 7, 3, true);    // display 1 - version number
  ...
  for (int count = 0; count < 8; count++)
  {
    ...
    // Check if button 1 held down if so go into demo mode
