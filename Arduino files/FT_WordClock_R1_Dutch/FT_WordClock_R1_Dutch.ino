...
void setup() {
  ...
  Serial.println("Tempus Fugit WordClock Application DUTCH version");
  ...
  */
  ...
  lc.setLed(0, 5, 7, true);   // Display IN
  ...
  while (count < 1200)  // setup for 2 min delay
  {
    ...
    */
    ...
    if (digitalRead(button3) == LOW) {
      ...
      if (bright_lim > 15) {
        bright_lim = 15;
        lc.setLed(0, 4, 5, true);     // flash H
        delay(250);
        lc.setLed(0, 4, 5, false);
      }
      lc.setLed(0, 2, 5, true);     // flash o  to indicate every push or S2 or S3
      delay(250);
